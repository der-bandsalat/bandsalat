...
export const GET: RequestHandler = async ({ url, cookies }) => {
	...
	if (!verified || verified.action !== 'extend') throw error(401, 'Ungültiger Token');
	...
	if (!user || !user.active) throw error(500, 'Demo-User fehlt');
	...
	throw redirect(303, '/?demo-extended=1');
};
